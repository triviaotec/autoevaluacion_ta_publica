...
# ───────── Compatibilidad con Streamlit 1.x / 2.x ─────────
...
# Logo
P_LOGO = next(
    (BASE / f"TRIVIA{ext}" for ext in (".png", ".jpeg") if (BASE / f"TRIVIA{ext}").exists()),
    None
)
...
def _valid_inputs() -> bool:
    if esc != 1:          # 2-5
        return True
    if disp is None:      # no respondió primera pregunta
        return False
    if disp == "No":
        return True
    if act is None:
...
with st.form(key=f"form_{it}"):
    ...
    if esc == 1:
        disp = st.radio(
            "1⃣ ¿Información disponible?", ["Sí", "No"],
            index=_safe_idx(["Sí","No"], prev.gen[0] if prev else None),
            key=key("disp")
        )
        if disp == "Sí":
            act = st.radio(
                "2⃣ ¿Información actualizada?", ["Sí", "No"],
    ...
    if infr:
        ...
        for m in ORDER_MAT:
            ...
            for it,lst in infr[m].items():
                p_it = doc.add_paragraph(f"  {it}")
                p_it.runs[0].bold=True
    ...
    if "mat_sc" not in st.session_state:
        ...
        for it,ans in st.session_state.answers.items():
            ...
            for idx,val in enumerate(ans.spec):
                if val=="No" and idx<len(textos):
                    infr[mat][it].append(f"Indicador «{textos[idx]}» = No")
