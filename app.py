...
# Logo
P_LOGO = next((BASE / f"TRIVIA{ext}" for ext in (".png", ".jpeg") if (BASE / f"TRIVIA{ext}").exists()), None)
...
def _puntaje_item(r: ItemR) -> float | None:
    ...
    esp_score=100 if not esp else round(sum(v=="Sí" for v in esp)/len(esp)*100)
    return round(gen_score*0.75+esp_score*0.25,1)

@st.cache_data(show_spinner=False, hash_funcs={namedtuple: id})
def calcular(ans: dict):
    item_sc={it:_puntaje_item(r) for it,r in ans.items()}
    mat_sc={}
    for m in ORDER_MAT:
...
def _tabla(doc, headers, rows):
    ...
    for i,h in enumerate(headers):
        ...
        cell._tc.get_or_add_tcPr().append(parse_xml(f"<w:shd {nsdecls('w')} w:fill='000000'/>"))
    # filas
...
def _export(mat_sc,item_sc,glob,infr,org,evalua):
    ...
    for s in doc.sections:
        s.top_margin=s.bottom_margin=Cm(2.5); s.left_margin=s.right_margin=Cm(2.0)
    ...
    if infr:
        ...
        for m in ORDER_MAT:
            ...
            pm=doc.add_paragraph(); pm.add_run(m).bold=True
    ...
    if "mat_sc" not in st.session_state:
        ...
        for it,ans in st.session_state.answers.items():
            ...
            for idx,val in enumerate(ans.spec):
                if val=="No":
                    textos=IND_ESP.get(f"{mat} || {it}",[])
                    if idx<len(textos): infr[mat][it].append(f"Indicador «{textos[idx]}» = No")
        with st.spinner("Generando informe…"):
            fname=_export(st.session_state.mat_sc,
